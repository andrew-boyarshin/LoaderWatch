...
)
{
    ...
    RtlCopyMemory(DllPath,
        InjectionInfo->InjectionDllPath.Buffer,
        InjectionInfo->InjectionDllPath.Length);
    ...
    PVOID ApcArgument2 = (PVOID)InjectionInfo->InjectionDllPath.Length;
...
)
{
    ...
    while (NextEntry != &InjInfoListHead)
    {
        ...
        if (InjectionInfo->PathAllocated)
        {
            ExFreePoolWithTag(InjectionInfo->InjectionDllPath.Buffer, INJ_PATH_MEMORY_TAG);
        }
...
)
{
    ...
    else
    {
        ...
        CapturedInjectionInfo->PathAllocated = FALSE;
        RtlInitEmptyUnicodeString(&CapturedInjectionInfo->InjectionDllPath, NULL, 0);
...
)
{
    ...
    else
    {
        ...
        if (InjectionInfo->SuppressInjectionOnModuleLoad)
        {
            return;
        }
