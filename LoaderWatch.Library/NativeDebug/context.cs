...
namespace LoaderWatch.Library.NativeDebug
{
	...
	public static class ContextAllocator
    {
        ...
        static public INativeContext GenerateContext()
        {
			...
			var arch = GetArchitectureFromPid(mdbgPid);
			var context = GenerateContext(arch);
        ...
        static public ProcessorArchitecture GetArchitectureFromPid(int pid)
        {
            ...
            else
            {
                ...
                using (var hProcess = NativeMethods.OpenProcess(0x0400, false, pid))
                {
        ...
        IDisposable
    {
		...
		/// <exception cref="InvalidOperationException">Throws if the architecture doesn't support single-stepping.</exception>
		...
		/// <exception cref="InvalidOperationException">Throws if no matching name or if register is not valid for the given Flags.</exception>
